...
/***/ function(module, exports, __webpack_require__) {
	...
	var drawBarGraph = function drawBarGraph(bardata, labelx, labely) {
	    var margin = {
	        top: 30,
	        right: 30,
	        bottom: 40,
	    ...
	    var xScale = d3.scaleBand().domain(bardata.map(function (d) {
	        return d[labelx];
	    })).range([0, width]).padding(.2);
	    var yScale = d3.scaleLinear().domain([0, d3.max(bardata, function (d) {
	        return d[labely];
	    })]).range([height, 0]);

	    var svg = d3.select('#chart').append('svg').attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom);
	    var bars = svg.append("g").attr("class", "bars").attr("transform", "translate(" + margin.left + ", " + margin.top + ")").selectAll("rect").data(bardata).enter().append("g");
	    var chart = bars.append("rect").style("fill", function (d) {
	        return colorScale(d[labely]);
	    ...
	    var bartext = bars.append("text").text(function (d) {
	        return d[labely];
	    }).attr("text-anchor", "middle").attr("fill", function (d) {
	        return colorTextScale(d[labely]);
	    }).attr("x", function (d) {
	        return xScale(d[labelx]) + xScale.bandwidth() / 2;
	    }).attr("y", height).style("opacity", 0);

	    chart.transition().delay(function (d, i) {
	        return i * 20;
	    }).duration(2000).attr("height", function (d) {
	        return height - yScale(d[labely]);
	    }).attr("y", function (d) {
	        return yScale(d[labely]);
	    }).ease(d3.easeElastic);
	    bartext.transition().delay(function (d, i) {
	        return i * 20;
	    }).duration(2000).attr("y", function (d) {
	        return yScale(d[labely]) + 15;
	...
	d3.csv("data/State-GPA.csv").row(function (data) {
	    return {
	        state: data.State,
