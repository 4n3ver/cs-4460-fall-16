...
/***/ function(module, exports, __webpack_require__) {
	...
	var drawBarGraph = function drawBarGraph(bardata, label) {
	    var margin = {
	        top: 30,
	        right: 30,
	        bottom: 80,
	    ...
	    var colorTextScale = function colorTextScale(d) {
	        if (d < 1) {
	            return "#FAFAFA";
	        } else if (d < 2) {
	            return "#424242";
	    ...
	    var updateGraph = function updateGraph(bardata) {
	        ...
	        var joinedBarGraph = barGraph.selectAll(".bar").data(bardata, label.x);
	        var oldRects = joinedBarGraph.selectAll("g.bar rect");
	        var oldTexts = joinedBarGraph.selectAll("g.bar text");
	        joinedBarGraph.exit().transition().duration(300).attr("y", yScale(0)).attr("height", height - yScale(0)).style("fill-opacity", 1e-6).remove();
	        var bars = joinedBarGraph.enter().append("g").attr("class", "bar");
	        var rects = bars.append("rect").style("fill", function (d) {
	            return colorScale(label.y(d));
	        ...
	        .attr("width", xScale.bandwidth()).attr("x", function (d) {
	            return xScale(label.x(d));
	        });
	        var texts = bars.append("text").attr("text-anchor", "middle").attr("fill", function (d) {
	            return colorTextScale(label.y(d));
	        }).attr("y", height).text(label.y).style("opacity", 0)
	        //.merge(oldTexts)
	        .attr("x", function (d) {
	            return xScale(label.x(d)) + xScale.bandwidth() / 2;
	        });
	        rects.transition().delay(function (d, i) {
	            return i * 20;
	        }).duration(2000).attr("height", function (d) {
	            return height - yScale(label.y(d));
	        }).attr("y", function (d) {
	            return yScale(label.y(d));
	        }).ease(d3.easeElastic);
	        oldRects.transition().delay(function (d, i) {
	            return i * 20;
	        ...
	        texts.transition().delay(function (d, i) {
	            return i * 20;
	        }).duration(2000).attr("y", function (d) {
	            return yScale(label.y(d)) + 15;
	        }).style("opacity", 1).ease(d3.easeElastic);
	        oldTexts.transition().delay(function (d, i) {
	            return i * 20;
	        ...
	        xAxis.transition().duration(300).call(d3.axisBottom(xScale)).selectAll("g g.tick > text").attr("text-anchor", "end").attr("transform", "rotate(-90) translate(" + -10 + ", -10)");
	...
	var setFilter = function setFilter(departments, onFilter) {
	    var filterDepartment = d3.select("#filter-dpt");
	    var filterGPA = d3.select("#filter-gpa");
	    ...
	    filterGPA.on("input", function () {
	        this.value = this.value.length > 0 ? Number.parseFloat(this.value) : null;
	    ...
	    filterButton.on("click", function () {
	        ...
	        onFilter(function (e) {
	            return e.gpa >= gpa && (dpt === "ALL" || e.department === dpt);
	        });
	    });
	    departments.forEach(function (e) {
	        return filterDepartment.append("option").attr("value", e).text(e);
	    });
	};
	
	d3.csv("data/Courses.csv").row(function (data) {
	    return {
	    ...
	    parsedData = parsedData.filter(function (d) {
	        return !Number.isNaN(d) && d.gpa >= 0 && d.gpa <= 4 && d.department.length > 0 && d.course.length > 0;
	    });
	
	    // build array of unique department
	    var departments = Object.keys(parsedData.reduce(function (prev, curr) {
	        prev[curr.department.toUpperCase()] = null;
	        return prev;
	    }, {}));
	
	    var updateBarGraph = drawBarGraph(parsedData, {
	        x: function x(d) {
	            return d.department + " " + d.course;
	        },
	        y: function y(d) {
	            return d.gpa;
	        }
	    });
	    setFilter(departments, function (comparator) {
	        return updateBarGraph(parsedData.filter(comparator));
	    });
	});
